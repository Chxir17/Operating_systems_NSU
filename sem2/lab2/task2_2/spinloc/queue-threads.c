//Спинлок (spinlock) — это примитив синхронизации, который обеспечивает взаимное исключение
//(как mutex), но работает активным ожиданием:
//если поток не может захватить блокировку — он не засыпает, а крутится в цикле ("спинится"), постоянно пытаясь захватить её.
